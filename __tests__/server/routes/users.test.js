...
describe('Users route', () => {
  ...
  const preSave = { email: 'mr.test@gmail.com', password: faker.internet.password() };
    ...
    it('should return 403 if email was found', done => {
      chai
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.be.deep.equal({ error: 'Email is already used' });
          done();
        });
    });
  });
  ...
  describe('secret', () => {
    ...
    it('should return status 200', done => {
      chai
        ...
        .end((err, res) => {
          expect(res.status).to.equal(200);
          expect(res.body).to.deep.equal({ secret: 'ressource' });
          done();
        });
    });
  });
